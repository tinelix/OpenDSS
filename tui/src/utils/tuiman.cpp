...
static BOOL CALLBACK FindCmdWindowProc() {
	HWND hWnd;
...
HWND TUIManager::findCmdWindow() {
		HWND hCmdWnd = NULL;
		EnumWindows(FindCmdWindowProc, NULL);
		return hCmdWnd;
}

#endif

void TUIManager::syncColors() {
	// Workaround for Windows NT 3.1 and 3.5
	#ifdef _WIN32
