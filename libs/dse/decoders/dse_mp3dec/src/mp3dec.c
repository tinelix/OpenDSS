...
LIB_EXPORT int DSE_OpenInputFile(const char f_path[512]) {
    ...
    size_t bytes_read = fread(_in_buf, 1, MAX_BLOCK_SIZE, _in_f);
    ...
    int result = dse_mp3dec_read_header(&mpeges, _in_buf, MAX_BLOCK_SIZE);
...
LIB_EXPORT int DSE_DecodeFrameSS(short** pcm_buf, size_t pcm_len) {
    ...
    while (_in_bytes_left > 0) {
        ...
        for (int line = 0; line < si.sfb; line++) {
        ...
        for (int i = 0; i < pcm_len; i++) {
            double normalized_value = time_domain_output[i] * 32767.0f;
